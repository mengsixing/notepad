...
const sortDateDesc = (a: IdataTableItem, b: IdataTableItem): number => {
  const aTime =
    a.startDate === '未开始' ? new Date().getTime() : Date.parse(a.startDate);
  const bTime =
    b.startDate === '未开始' ? new Date().getTime() : Date.parse(b.startDate);
  return aTime - bTime;
};
      ...
      for (let i = 0; i < allData.length; i++) {
        dataSource.push({
          ...
          startDate: allData[i].startDate
            ? createDateString(new Date(allData[i].startDate))
            : '未开始',
          finishDate: allData[i].finishDate
            ? createDateString(new Date(allData[i].finishDate))
            : '未完成',
        });
      }
      this.setState({
